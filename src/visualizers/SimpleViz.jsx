...
export default function SimpleViz() {
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        .sv-cell { transition: all 0.3s ease; }
        .sv-btn { cursor:pointer; border:none; font-family:inherit; transition: all 0.15s; }
        .sv-btn:hover { transform:translateY(-1px); }
        .sv-btn:active { transform:translateY(0); }
      `}</style>

      <div style={{ textAlign: "center", marginBottom: 36 }}>
        <div style={{ fontSize: 11, letterSpacing: 6, color: "#5a5a6a", marginBottom: 8, textTransform: "uppercase" }}>Quick Visual Mode</div>
      ...
      <div style={{ display: "flex", gap: 8, justifyContent: "center", marginBottom: 32, flexWrap: "wrap" }}>
        ...
        }} style={{ background: "#2a2aff", color: "#fff", padding: "8px 18px", borderRadius: 6, fontSize: 13 }}>Load Array</button>
        <button className="sv-btn" onClick={() => {
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: 6, marginBottom: 8, flexWrap: "wrap" }}>
        {arr.map((v, i) => {
          ...
          return (
            <div key={i} className="sv-cell" style={{
      ...
      <div style={{
        ...
        padding: "16px 20px", animation: "slideIn 0.3s ease", minHeight: 52,
      }}>
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
        <button className="sv-btn" onClick={() => { setPlaying(false); setStepIdx(0); }} style={{ background: "#1a1a2e", color: "#8888bb", padding: "9px 16px", borderRadius: 6, border: "1px solid #2a2a3a", fontSize: 13 }}>⏮ Reset</button>
        <button className="sv-btn" onClick={() => setStepIdx(s => Math.max(0, s - 1))} style={{ background: "#1a1a2e", color: "#8888bb", padding: "9px 16px", borderRadius: 6, border: "1px solid #2a2a3a", fontSize: 13 }}>← Prev</button>
        <button className="sv-btn" onClick={() => setPlaying(p => !p)} style={{
          background: playing ? "#4a1a1a" : "#2a2aff", color: "#fff",
          padding: "9px 22px", borderRadius: 6, fontSize: 13, minWidth: 90,
        }}>{playing ? "⏸ Pause" : "▶ Play"}</button>
        <button className="sv-btn" onClick={() => setStepIdx(s => Math.min(steps.length - 1, s + 1))} style={{ background: "#1a1a2e", color: "#8888bb", padding: "9px 16px", borderRadius: 6, border: "1px solid #2a2a3a", fontSize: 13 }}>Next →</button>
        <button className="sv-btn" onClick={() => { setPlaying(false); setStepIdx(steps.length - 1); }} style={{ background: "#1a1a2e", color: "#8888bb", padding: "9px 16px", borderRadius: 6, border: "1px solid #2a2a3a", fontSize: 13 }}>⏭ End</button>
      </div>
